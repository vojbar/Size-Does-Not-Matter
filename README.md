...
The UniFi Switch Ultra 210W is mounted on the upper shelf of the rack and provides both network connectivity and PoE for all four Raspberry Pi nodes. With a 210W PoE budget, the switch can power all four nodes simultaneously while maintaining a significant amount of available capacity even under high load. Directly below the switch, the Ethernet Patch Panel keeps the network connections organized and the front of the rack clean. Two SBC shelves hold the four Raspberry Pi 5 4GB nodes, with two nodes on each shelf. Each Raspberry Pi is equipped with a PoE+ Module for power and network connectivity, as well as an NVMe storage module connected through the Raspberry Pi 5's PCIe interface. The 256 GB NVMe SSD is used as the primary boot drive and provides additional local storage. A Brush Panel at the bottom of the rack provides a clean path for the remaining cabling and keeps it out of sight.
